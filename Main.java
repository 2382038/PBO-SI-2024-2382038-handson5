...
public class Main {
    ...
    public static boolean isSelectedTodoNotValid(Integer number) {
        //check if the number is zero or less then zero
        if (number <= 0) {
            return true;
        }
        //check if the number id greater than the todos size/length
        if (number - 1 > todos.length - 1) {
            return true;
        }

        //check wether the element is already null
